...
public class Book : MonoBehaviour {
    ...
    void Init()
    {
        flip = transform.parent.GetComponent<FlipEffectManager>();

        // Deactivate all registered pm
        flip.DeactivateAllPM();

        Texture2D texture = flip.transform.parent.GetComponent<PaintManager>().GetResultTexture();
        flip.ApplyTexture(texture);
    ...
    void Flip()
    {
        // Destroy Gameobject on proper flip
        ...
        else
        {
            currentCoroutine = StartCoroutine(TweenTo(ebl, 0.15f,
                () =>
                {
                    ...
                    LeftNext.transform.SetParent(flip.controller.transform);
                    Left.transform.SetParent(flip.controller.transform);
